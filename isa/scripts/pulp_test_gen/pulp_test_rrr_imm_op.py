...
class pulp_test_rrr_uimm5_op(pulp_test_op):
    def __init__(self, mnemonic: str, res_format = "0x{:08x}", src1_format = "0x{:08x}", 
                 src2_format = "0x{:08x}", src3_format = "0x{:08x}", imm1_format = "0x{:02x}"):
        '''
            Implements the test-generator of a generic operation 
            (three reg + one 5bit uint immediate operands)
        ...
        self.minmax.append((0, 0x1F)) # imm1

        args_format = OrderedDict([("testnum", "{:d}"), ("op", "{:s}"), 
                                   ("res", res_format), ("src1", src1_format), ("src2", src2_format), 
                                   ("src3", src3_format), ("imm1", imm1_format)])
        self.arith_macro = test_macro("TEST_RRR_UIMM5_OP", args_format)

        args_format.pop("src3") # because src3 is also dest register
        self.src_dest_macros = [test_macro("TEST_RRR_UIMM5_SRC1_EQ_DEST", args_format), # enforce src1 = src3!
                                test_macro("TEST_RRR_UIMM5_SRC2_EQ_DEST", args_format)] # enforce src2 = src3!

        args_format.pop("src2")
        self.src_dest_macros.append(test_macro("TEST_RRR_UIMM5_SRC12_EQ_DEST", args_format)) # enforce src1 = src2 = src3!
        ...
        self.bypass_macros = [test_macro("TEST_RRR_UIMM5_SRC12_BYPASS", args_format),
                              test_macro("TEST_RRR_UIMM5_SRC21_BYPASS", args_format)]
        args_format.pop("nops2")
        self.bypass_macros.append(test_macro("TEST_RRR_UIMM5_DEST_BYPASS", args_format))
        self.bypass_macros.append(test_macro("TEST_RRR_UIMM5_SRC3_BYPASS", args_format))
        ...
        self.zero_reg_macros = [test_macro("TEST_RRR_UIMM5_ZEROSRC1", args_format)]

        args_format = OrderedDict([("testnum", "{:d}"), ("op", "{:s}"), 
                                   ("res", res_format), ("src1", src1_format), 
                                   ("src3", src3_format), ("imm1", imm1_format)])
        self.zero_reg_macros.append(test_macro("TEST_RRR_UIMM5_ZEROSRC2", args_format))
        
        args_format = OrderedDict([("testnum", "{:d}"), ("op", "{:s}"), 
                                   ("res", res_format), ("src1", src1_format), 
                                   ("src2", src2_format), ("imm1", imm1_format)])
        self.zero_reg_macros.append(test_macro("TEST_RRR_UIMM5_ZEROSRC3", args_format))

        args_format = OrderedDict([("testnum", "{:d}"), ("op", "{:s}"), 
                                   ("res", res_format), ("src3", src3_format), ("imm1", imm1_format)])
        self.zero_reg_macros.append(test_macro("TEST_RRR_UIMM5_ZEROSRC12", args_format))
        args_format.pop("src3")
        self.zero_reg_macros.append(test_macro("TEST_RRR_UIMM5_ZEROSRC123", args_format))
        
        args_format = OrderedDict([ ("testnum", "{:d}"), ("op", "{:s}"), 
                                    ("src1", src1_format), ("src2", src2_format), ("imm1", imm1_format)])
        self.zero_reg_macros.append(test_macro("TEST_RRR_UIMM5_ZERODEST", args_format))
    ...
    def operation(self, src1: int, src2: int, src3: int, imm1: int) -> int:
        '''
            ...
            Parameters:
                ...
                imm1 (int): Value of first immediate operand (5bit unsigned)
    ...
    def add_arith_test(self, src1: int, src2: int, src3: int, imm1: int):
        '''
        ...
        args = {"testnum": self.testnum, "op": self.mnemonic,
                "res": self.operation(src1, src2, src3, imm1), 
                "src1": src1, "src2": src2, "src3": src3, "imm1": imm1}
    ...
    def gen_arith_tests(self, num: int):
        ...
        for i in range(num):
            ...
            imm1 = random.randint(*self.minmax[3])
            self.add_arith_test(src1, src2, src3, imm1)
    ...
    def gen_src_dest_tests(self, num_per: int):
        ...
        for macro in self.src_dest_macros: # (&dest) == (&src1) and (&dest) == (&src2) 
            for i in range(num_per):
                ...
                args["imm1"] = random.randint(*self.minmax[3])
                args["res"] = self.operation(args["src1"], args["src2"], args["src3"], args["imm1"])
