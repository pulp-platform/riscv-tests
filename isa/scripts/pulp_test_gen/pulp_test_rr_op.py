...
class pulp_test_rr_op(pulp_test_op):
    def __init__(self, mnemonic: str, res_format = "0x{:08x}", src1_format = "0x{:08x}", src2_format = "0x{:08x}"):
        '''
            Implements the test-generator of a generic operation (two reg operands)
        ...
        self.bypass_macros = [test_macro("TEST_RR_SRC12_BYPASS", args_format),
                              test_macro("TEST_RR_SRC21_BYPASS", args_format)]
        args_format.pop("nops2")
        self.bypass_macros.append(test_macro("TEST_RR_DEST_BYPASS", args_format))


        args_format = OrderedDict([ ("testnum", "{:d}"), ("op", "{:s}"), 
                                    ("res", res_format), ("src2", src2_format)])

        self.zero_reg_macros = [test_macro("TEST_RR_ZEROSRC1", args_format)]
        args_format.pop("src2")
        args_format["src1"] = src1_format
        self.zero_reg_macros.append(test_macro("TEST_RR_ZEROSRC2", args_format))
        args_format.pop("src1")
        self.zero_reg_macros.append(test_macro("TEST_RR_ZEROSRC12", args_format))
        
        args_format = OrderedDict([ ("testnum", "{:d}"), ("op", "{:s}"),
                                    ("src1", src1_format), ("src2", res_format)])
        self.zero_reg_macros.append(test_macro("TEST_RR_ZERODEST", args_format))
    ...
    def add_arith_test(self, src1: int, src2: int):
        '''
        ...
        args = {"testnum": self.testnum, "op": self.mnemonic,
                "res": self.operation(src1, src2), "src1": src1, "src2": src2}
    ...
    def gen_src_dest_tests(self, num_per: int):
        ...
        for macro in self.src_dest_macros: # (&dest) == (&src1) and (&dest) == (&src2) 
            for i in range(num_per):
                ...
                args["res"] = self.operation(args["src1"], args["src2"])
